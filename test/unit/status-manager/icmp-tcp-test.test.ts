...
describe('IcmpTcpTest', () => {
	...
	const emitIsProxy = (isProxy: boolean | null) => socketEvents.emit('api:connect:isProxy', { isProxy });
	...
	it('should not block if first run exceeds threshold but second run does not', async () => {
		...
		pingCmd.callsFake(async () => {
			// First 3 calls (first run): avg=110 → diff=100; rest: avg=20 → diff=10
			return { stdout: makeIcmpOutput(icmpCallNum++ < 3 ? 110 : 20) };
		});
